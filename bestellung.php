...
while ($row = mysql_fetch_array($result)) {
    ...
    $query3 = "Select Artikelnummer,Name,Bild,Bestand from artikel where artikelnummer='$row[Artikelnummer]'";
    ...
    while ($row2 = mysql_fetch_array($result3)) {
        ...
        if ($row["Anzahl"] > $row2["Bestand"]) {
            echo "Ihre Bestellung konnte nicht angenommen werden.<br> ";
        ...
        if ($result2 and $result5) {
            if ($test == "0") {
                echo "<h1 align=\"center\" >Erfolgreiche Bestellung</h1>";
                $test++;
            }
            ...
            if ($test == "1") {
                echo "<tr><td>Artikelnummer</td><td>Name</td><td>Bild</td><td>Anzahl</td>";
            }
     ...
     <td>$row[Anzahl]</td>
        ...
        if (!$result4) {
            echo "Fehler bei der Löschung der Artikel aus dem Warenkorb.";
        }
